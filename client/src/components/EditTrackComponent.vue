<template>
   <div class="edit-box">
      <form>
         ...
         <p
            class="tag"
            v-if="track.artists.length != 0"
            style="margin-top: 6px; margin-bottom: 4px"
         >
            Enter artist USERNAME
         </p>
         <span
            class="artist-entry"
            v-for="(artist, index) in track.artists.slice(1)"
            :key="index"
         >
            <input
               class="artist"
               type="text"
               v-model="track.artists[index + 1]"
               placeholder="a very cool person"
            />
            <span
               class="material-symbols-rounded remove-artist"
               @click="remove_artist(index + 1)"
               >cancel</span
            >
         </span>
         <span
...
const emit_album_cover = async (e) => {
   ...
   console.log(
      `Compressed album from ${orig.size / 1024}kb to ${album.size / 1024}kb.`
   )
